...
const useJobs = () => {
  ...
  const handleReceiptVerification = (receiptData) => {
    receipts.setCurrentReceiptData(receiptData);
    receipts.setShowVerificationModal(false);
    receipts.setShowReceiptModal(true);
  };

  // View receipts
  const viewReceipts = (job) => {
    receipts.setSelectedJobForReceipt(job);
    receipts.setShowReceiptModal(true);
  };
